class Commander{
    ...
    fill(structure, config){
        let cmd_queue = [];
        let {block2, data2, method} = config;
        let rep_block = method === 'replace' ? [block2, data2].join(' ') : null;
        if(!Array.isArray(structure[0]))return;
    ...
    summon(structure, config){
        let cmdVec = []
        for (let i of structure){
            cmdVec.push(`summon ${config.entity} ${i.join(' ')}`)
        }
        return cmdVec;
    }
    ...
    clone(structure, config){
        let cmd_queue = [];
        if(!Array.isArray(structure[0]))return;
        let {begin, end, method, method2, block2, data2} = config;
        if (!begin || !end){
            return {err:'Range error(Begin and end).'}
        }
        method = method ? method : '';
        method2 = method2 ? method2 : '';
        let block = (block2 || '') + ' ' + (data2 || '');
        let mode = method !== 'filtered' ? method + method2 : method + method2 + block;
        for(let i of structure){
            cmd_queue.push(`clone ${begin.join(' ')} ${end.join(' ')} ${i.join(' ')} ${mode}`)
        }
        return cmd_queue;
    }
    ...
    setBlockSplit(chunks, c){
        ...
        for (let [_,l] of chunks){
            for (let [_,w] of l){
                ...
                cmdVec = cmdVec.concat(this[c.gen](w, c))
            }
        }
        return cmdVec;
    }
